...
func (d *disassembler) sizeOfTypeInBits(t types.Type) int64 {
	switch t := t.(type) {
	case *types.VoidType:
	...
	case *types.VectorType:
		return t.Len * d.sizeOfTypeInBits(t.Elem)
	...
	case *types.StructType:
		...
		for _, field := range t.Fields {
			total += d.sizeOfTypeInBits(field)
		}
		return total
	default:
...
func (d *disassembler) sizeOfType(t types.Type) int64 {
	bits := d.sizeOfTypeInBits(t)
	if bits%8 != 0 {
